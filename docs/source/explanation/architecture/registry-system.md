...
<a id="component-architecture-c4-model-component-level"></a>
...
The diagram below shows how {py:class}`~bluetooth_sig.core.translator.BluetoothSIGTranslator`, {py:class}`~bluetooth_sig.gatt.characteristics.registry.CharacteristicRegistry`, and {py:class}`~bluetooth_sig.gatt.uuid_registry.UuidRegistry` work together to resolve UUIDs:
...
The {py:class}`~bluetooth_sig.gatt.uuid_registry.UuidRegistry` class is a **singleton** that:
...
The {py:class}`~bluetooth_sig.gatt.uuid_registry.UuidRegistry` maintains several internal storage structures optimized for fast lookups:
...
This dual-storage approach (canonical + aliases) minimizes memory usage while supporting multiple lookup patterns. For complete implementation details, see the {py:class}`~bluetooth_sig.gatt.uuid_registry.UuidRegistry` source code.

### CharacteristicInfo Structure

The {py:class}`~bluetooth_sig.types.CharacteristicInfo` struct stores metadata about Bluetooth characteristics loaded from SIG specifications. It extends the base {py:class}`~bluetooth_sig.types.base_types.SIGInfo` type with characteristic-specific fields including value type and unit information.

See the {py:class}`~bluetooth_sig.types.CharacteristicInfo` API reference for the complete field list and usage examples.

### CharacteristicSpec Structure

For characteristics requiring detailed specifications, the {py:class}`~bluetooth_sig.types.gatt_services.CharacteristicSpec` struct provides enriched metadata including:
...
This richer specification is used when cross-referencing YAML files from the Bluetooth SIG GATT Specification Supplement (GSS). See the {py:class}`~bluetooth_sig.types.gatt_services.CharacteristicSpec` source code for complete details.
...
This pattern minimizes lock contention while guaranteeing thread safety. See {py:class}`~bluetooth_sig.gatt.uuid_registry.UuidRegistry` source code for implementation details.
...
This multi-strategy approach supports various identifier formats while maintaining O(1) lookup performance. See {py:meth}`~bluetooth_sig.gatt.uuid_registry.UuidRegistry.get_characteristic_info` for implementation details.

## BaseRegistry Pattern

**Location**: `src/bluetooth_sig/registry/base.py`

All specialized registries inherit from {py:class}`~bluetooth_sig.registry.base.BaseUUIDRegistry`, which provides:
...
The {py:class}`~bluetooth_sig.registry.base.BaseUUIDRegistry` pattern provides two key guarantees:
...
The {py:class}`~bluetooth_sig.core.translator.BluetoothSIGTranslator` provides methods to register custom UUIDs at runtime:
...
3. **Create info object**: Build {py:class}`~bluetooth_sig.types.CharacteristicInfo` or {py:class}`~bluetooth_sig.types.ServiceInfo` with runtime origin marker
