...
class PulseOximetryMeasurementCharacteristic(BaseCharacteristic):
    """Pulse Oximetry Measurement characteristic (0x2A5F).
    
    Used to transmit SpO2 (blood oxygen saturation) and pulse rate measurements.
    """

    _characteristic_name: str = "Pulse Oximetry Measurement"
    ...
    def parse_value(self, data: bytearray) -> Dict[str, Any]:
        ...
        if (flags & 0x04) and len(data) >= offset + 3:
            device_status = struct.unpack("<I", data[offset:offset + 3] + b'\x00')[0]  # Pad to 4 bytes
