...
from ...types.gatt_enums import CharacteristicName, CharacteristicRole, DataType, GattProperty, ValueType
...
class BaseCharacteristic(ABC, Generic[T], metaclass=CharacteristicMeta):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def role(self) -> CharacteristicRole:
        ...
        Subclasses can bypass the heuristic entirely by setting
        ``_manual_role`` as a class variable::

            class MyCharacteristic(BaseCharacteristic[int]):
                _manual_role = CharacteristicRole.MEASUREMENT

        When ``_manual_role`` is ``None`` (the default), the role is inferred
        from SIG metadata using the heuristic described below.

        The result is cached on the **class** — every instance of the same
        characteristic type shares a single lookup, because the classification
        depends only on class-level metadata (name, value_type, unit, spec).

        Classification priority (first match wins):
            0. ``_manual_role`` is set                   → that value
            ...
            5. Compound type (VARIOUS / DICT) with a
               characteristic-level unit or field-level
               ``unit_id``                              → MEASUREMENT
        ...
        if cls._cached_role is None:
            # Honour explicit override before falling back to heuristic
            if cls._manual_role is not None:
                cls._cached_role = cls._manual_role
            else:
                cls._cached_role = self._classify_role()
        return cls._cached_role

    def _classify_role(self) -> CharacteristicRole:
        """Compute the role from SIG spec metadata (called once per class)."""
        char_name = self.name
        ...
        if (
            char_name.endswith("Feature")
            or char_name.endswith("Features")
            or self.value_type == ValueType.BITFIELD
        ):
        ...
        if self.value_type in (ValueType.INT, ValueType.FLOAT) and self.unit:
            return CharacteristicRole.MEASUREMENT

        # 5. Compound value with unit metadata (char-level or per-field)
        if self.value_type in (ValueType.VARIOUS, ValueType.DICT) and (
            self.unit or self._spec_has_unit_fields()
        ):
            return CharacteristicRole.MEASUREMENT
    ...
    def _spec_has_unit_fields(self) -> bool:
        """Check whether any field in the GSS spec carries a ``unit_id``.

        Returns ``True`` if the characteristic's resolved GSS specification
        contains at least one :class:`FieldSpec` with a non-empty
        ``unit_id``, indicating that the field represents a physical
        quantity with a defined unit of measurement.
        """
        if not self._spec:
            return False
        structure = getattr(self._spec, "structure", None)
        if not structure:
            return False
