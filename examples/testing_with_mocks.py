...
def run_basic_parsing_tests() -> dict[str, Any]:
    ...
    for uuid, test_data in MockCharacteristicData.SIG_COMPLIANT_DATA.items():
        ...
        # Test valid data
        for raw_data, expected_value, _expected_unit in test_data["valid"]:
            ...
            try:
                ...
                if result.parse_success:
                    ...
                    if result.value == expected_value:
                        print(
                            f"   ✅ Valid data: {raw_data.hex()} → {result.value} {result.unit or ''}"
                        )
                        results["passed"] += 1
        ...
        for raw_data, _expected_error in test_data["invalid"]:
            results["total_tests"] += 1
            try:
                result = translator.parse_characteristic(uuid, raw_data)
                ...
                if not result.parse_success:
                    print(
                        f"   ✅ Invalid data handled: {raw_data.hex() if raw_data else 'empty'}"
                    )
                    results["passed"] += 1
                    char_results["invalid_handled"] += 1
                else:
                    print(
                        f"   ⚠️  Invalid data accepted: {raw_data.hex() if raw_data else 'empty'}"
                    )
                    results["failed"] += 1
...
def run_performance_benchmark() -> dict[str, Any]:
    ...
    print(
        f"   📦 Batch parse ({len(batch_data)} items): {results['batch_parse_time']:.3f}ms"
    )
        ...
        if uuid in MockCharacteristicData.SIG_COMPLIANT_DATA:
            ...
            if result.parse_success:
                if type(result.value).__name__ == result.value_type:
                    print(
                        f"     ✅ {uuid}: Value type consistent ({result.value_type})"
                    )
                    results["passed"] += 1
                else:
                    print(
                        f"     ⚠️  {uuid}: Value type mismatch (got {type(result.value).__name__}, expected {result.value_type})"
                    )
...
def run_comprehensive_test_suite():
    ...
    # Summary
    total_tests = (
        basic_results["total_tests"]
        + edge_results["total_tests"]
        + compliance_results["total_checks"]
    )
    total_passed = (
        basic_results["passed"] + edge_results["passed"] + compliance_results["passed"]
    )
    total_failed = (
        basic_results["failed"] + edge_results["failed"] + compliance_results["failed"]
    )
...
def main():
    ...
    parser.add_argument(
        "--compliance", action="store_true", help="Run compliance validation only"
    )
