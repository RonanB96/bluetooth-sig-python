...
class ComparisonAnalyzer:
    ...
    def compare_parsing(self, characteristic_uuid: str, data: bytes) -> ParseComparison:
        ...
        sig_error = None
        try:
            sig_parse_result = self.sig_translator.parse_characteristic(characteristic_uuid, data)
    ...
    def _analyze_features(self, manual_result: Dict, sig_result: Any) -> Dict[str, Any]:
        ...
        if manual_result and not manual_result.get("error"):
            ...
            features["code_complexity"]["manual"] = "High - custom parsing logic required"
        ...
        if sig_result and sig_result.parse_success:
            ...
            features["standards_compliance"]["sig"] = "Full - official SIG specifications"
...
async def run_comparison_with_device(address: str, timeout: float = 10.0) -> None:
    ...
    try:
        async with BleakClient(address, timeout=timeout) as client:
            ...
            test_characteristics = await find_comparable_characteristics(client, analyzer.sig_translator)
...
async def find_comparable_characteristics(client, translator: BluetoothSIGTranslator) -> Dict[str, Any]:
...
def display_summary(comparisons: list[ParseComparison]) -> None:
    ...
    manual_successes = sum(1 for c in comparisons if c.manual_result and not c.manual_error)
    sig_successes = sum(1 for c in comparisons if c.sig_result and not c.sig_error)
    
    print(f"\n📈 Success Rates:")
    print(f"   Manual Parsing: {manual_successes}/{total_tests} ({manual_successes/total_tests*100:.1f}%)")
    print(f"   SIG Library: {sig_successes}/{total_tests} ({sig_successes/total_tests*100:.1f}%)")
    ...
    print(f"   ✅ Higher success rate: {sig_successes - manual_successes} more successful parses")
    print(f"   ✅ Standards compliance: Full SIG specification implementation")
    print(f"   ✅ Rich data extraction: {total_missing_features} additional features extracted")
    print(f"   ✅ Lower complexity: Single method call vs custom parsing logic")
    print(f"   ✅ Error handling: Comprehensive validation and error reporting")
    print(f"   ✅ Future-proof: Automatic updates with SIG specification changes")
    
    print(f"\n🔧 Manual Parsing Limitations:")
    print(f"   ❌ Custom code required for each characteristic")
    print(f"   ❌ Easy to introduce bugs in bit manipulation")
    print(f"   ❌ Missing advanced features (timestamps, status flags, etc.)")
    print(f"   ❌ No automatic validation")
    print(f"   ❌ Manual maintenance required for spec updates")
...
def run_benchmark_comparison() -> None:
    ...
    for uuid, data in test_data:
        ...
        print(f"\n📊 {char_name}:")
        print(f"   Manual: {manual_time*1000:.2f}ms ({manual_time/iterations*1000000:.1f}μs per parse)")
        print(f"   SIG Library: {sig_time*1000:.2f}ms ({sig_time/iterations*1000000:.1f}μs per parse)")
