...
class SIGIntegrationGuide:
    ...
    def demonstrate_lookup_patterns(self) -> None:
        ...
        if service_info and char_info:
            print(f"Result: Found {service_info.name} ({service_info.uuid}) -> {char_info.name} ({char_info.uuid})")
    ...
    def demonstrate_production_patterns(self) -> None:
        ...
        print("        battery_service = find_service_by_uuid(client, service_info.uuid)")
        ...
        print("        battery_char = find_characteristic_by_uuid(battery_service, char_info.uuid)")
        ...
        print("        result = translator.parse_characteristic(char_info.uuid, raw_data)")
        print("")
    ...
    def demonstrate_performance_patterns(self) -> None:
        ...
        print("            self._service_cache[name] = self.translator.get_service_info_by_name(name)")
    ...
    def demonstrate_error_patterns(self) -> None:
        ...
        print("```python")
        print("async def read_with_graceful_degradation(client, translator, service_name, char_name):")
        print("    try:")
        print("        # Try standard approach")
        print("        result = await read_characteristic_by_name(client, translator, service_name, char_name)")
        print("        if result and result.parse_success:")
        ...
        print("        char_info = translator.get_characteristic_info_by_name(char_name)")
        print("        if char_info:")
        print("            raw_data = await read_characteristic_by_uuid(client, char_info.uuid)")
        ...
        print("```python")
        print("def validate_parsed_result(result, expected_type=None, expected_range=None):")
...
async def demonstrate_with_mock_device() -> None:
    ...
    for service_uuid, service_data in mock_device["services"].items():
        ...
        for char_uuid, char_data in service_data["characteristics"].items():
            print(f"    📄 {char_data['name']} ({char_uuid}) - {char_data['properties']}")
    ...
    for service_uuid in mock_device["services"].keys():
        ...
        if service_info:
            ...
            for char_uuid in service_data["characteristics"].keys():
                char_info = guide.translator.get_characteristic_info_by_uuid(char_uuid)
                if char_info:
                    print(f"    📄 {char_uuid} -> Known SIG characteristic: {char_info.name}")
                else:
        ...
        async with BleakClient(address, timeout=timeout) as client:
            ...
            for service in client.services:
                ...
                for char in service.characteristics:
                    char_info = guide.translator.get_characteristic_info_by_uuid(char.uuid)
                    
                    if char_info:
                        char_name = char_info.name
                        char_status = "✅ Known SIG"
                    else:
                        char_name = f"Unknown Characteristic"
...
async def read_characteristic_by_name(client, translator, service_name: str, char_name: str) -> Any:
...
async def main():
    ...
    parser.add_argument(
        "--address",
        help="BLE device address for real device demo"
    )
    parser.add_argument(
        "--scan",
        action="store_true",
