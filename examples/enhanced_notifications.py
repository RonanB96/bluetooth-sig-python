...
# pylint: disable=import-error,too-few-public-methods,broad-exception-caught,possibly-used-before-assignment
...
class SIGNotificationHandler:
        ...
        def sig_notification_handler(
            ...
            if char_info:
                ...
                if result.parse_success:
                    ...
                    logger.warning(
                        "❌ Notification #%d (%.1fs): %s - Parse failed: %s",
                        self.notification_count,
                        elapsed,
                        result.name,
                ...
                logger.warning(
                    "❓ Notification #%d (%.1fs): %s - Not a known SIG characteristic",
                    self.notification_count,
                    elapsed,
                    char_name,
                )
    ...
    def _display_heart_rate_details(self, result) -> None:
        ...
        if hasattr(result.value, "sensor_contact_detected"):
            contact_status = (
                "✅ Good" if result.value.sensor_contact_detected else "❌ Poor"
            )
    ...
    def _display_temperature_details(self, result) -> None:
        ...
        if hasattr(result.value, "temperature"):
            logger.info(
                "   🌡️  Temperature: %.2f %s", result.value.temperature, result.unit
            )
...
async def monitor_notifications_with_sig_parsing(
    ...
    try:
        async with BleakClient(address, timeout=timeout) as client:
            ...
            notifiable_chars = await find_notifiable_characteristics(
                client, translator, target_characteristic
            )
            ...
            subscribed_count = await subscribe_to_notifications(
                client, handler, notifiable_chars
            )
...
async def find_notifiable_characteristics(
    client, translator: BluetoothSIGTranslator, target_characteristic: str = None
) -> list:
        ...
        for char in service.characteristics:
            if "notify" in char.properties or "indicate" in char.properties:
                ...
                if target_characteristic:
                    ...
                    name_match = (
                        char_info
                        and char_info.name.lower() == target_characteristic.lower()
                    )
                ...
                notifiable_chars.append(
                    {"characteristic": char, "info": char_info, "name": char_name}
                )
...
async def subscribe_to_notifications(
    client, handler: SIGNotificationHandler, notifiable_chars: list
) -> int:
...
async def main():
    ...
    if not args.address:
        ...
        print(
            "  python enhanced_notifications.py --address 12:34:56:78:9A:BC --characteristic '2A37'"
        )
        print(
    ...
    await monitor_notifications_with_sig_parsing(
        args.address, args.characteristic, args.duration, args.timeout
    )
