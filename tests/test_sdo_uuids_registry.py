...
from bluetooth_sig.registry.uuids.sdo_uuids import SdoUuidsRegistry, get_sdo_uuids_registry
...
class TestSdoUuidsRegistry:
    ...
    def test_registry_initialization(self, sdo_uuids_registry: SdoUuidsRegistry) -> None:
        ...
        sdo_uuids = get_sdo_uuids_registry().get_all_sdo_uuids()
    ...
    def test_get_sdo_info(self, sdo_uuids_registry: SdoUuidsRegistry) -> None:
        ...
        info = get_sdo_uuids_registry().get_sdo_info("0xFFFE")
    ...
    def test_get_sdo_info_by_name(self, sdo_uuids_registry: SdoUuidsRegistry) -> None:
        ...
        info = get_sdo_uuids_registry().get_sdo_info_by_name("Wireless Power Transfer")
        if info:  # Only if YAML loaded
            assert isinstance(info, SdoInfo)
            assert info.name == "Wireless Power Transfer"
        ...
        info_lower = get_sdo_uuids_registry().get_sdo_info_by_name("wireless power transfer")
        assert info_lower == info

        # Test not found
        info_none = get_sdo_uuids_registry().get_sdo_info_by_name("Nonexistent SDO")
    ...
    def test_get_sdo_info_by_id(self, sdo_uuids_registry: SdoUuidsRegistry) -> None:
        ...
        info = get_sdo_uuids_registry().get_sdo_info_by_id("org.bluetooth.sdo.wireless_power_transfer")
        if info:  # Only if YAML loaded
            assert isinstance(info, SdoInfo)
            assert info.name == "Wireless Power Transfer"
            assert info.uuid.short_form.upper() == "FFFE"
        ...
        # Test not found
        info_none = get_sdo_uuids_registry().get_sdo_info_by_id("org.bluetooth.sdo.nonexistent")
        assert info_none is None
    ...
    def test_get_sdo_info_by_bluetooth_uuid(self, sdo_uuids_registry: SdoUuidsRegistry) -> None:
        ...
        info = get_sdo_uuids_registry().get_sdo_info(bt_uuid)
        if info:  # Only if YAML loaded
            assert isinstance(info, SdoInfo)
            assert info.name == "Wireless Power Transfer"
    ...
    def test_get_sdo_info_not_found(self, sdo_uuids_registry: SdoUuidsRegistry) -> None:
        ...
        info = get_sdo_uuids_registry().get_sdo_info("nonexistent")
        assert info is None

        info = get_sdo_uuids_registry().get_sdo_info("0x0000")  # Not an SDO UUID
        assert info is None

    def test_is_sdo_uuid(self, sdo_uuids_registry: SdoUuidsRegistry) -> None:
        """Test SDO UUID validation."""
        # Known SDO UUID
        has_sdos = bool(get_sdo_uuids_registry().get_all_sdo_uuids())
        assert get_sdo_uuids_registry().is_sdo_uuid("0xFFFE") or not has_sdos

        # Non-SDO UUID
        assert not get_sdo_uuids_registry().is_sdo_uuid("0x0000")

        # Invalid UUID
        assert not get_sdo_uuids_registry().is_sdo_uuid("invalid")
    ...
    def test_uuid_formats(self, sdo_uuids_registry: SdoUuidsRegistry) -> None:
        ...
        for fmt in formats:
            info = get_sdo_uuids_registry().get_sdo_info(fmt)
            if get_sdo_uuids_registry().is_sdo_uuid("FFFE"):
    ...
    def test_normalize_name_for_id(self, sdo_uuids_registry: SdoUuidsRegistry) -> None:
        ...
        for name, expected_suffix in test_cases:
            normalized = get_sdo_uuids_registry()._normalize_name_for_id(name)
