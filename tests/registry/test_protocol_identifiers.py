...
from bluetooth_sig.registry.uuids.protocol_identifiers import (
...
class TestProtocolIdentifiersRegistry:  # pylint: disable=too-many-public-methods
    ...
    def test_registry_initialization(self, protocol_identifiers_registry: ProtocolIdentifiersRegistry) -> None:
        ...
        protocols = get_protocol_identifiers_registry().get_all_protocols()
    ...
    def test_get_protocol_info_l2cap(self, protocol_identifiers_registry: ProtocolIdentifiersRegistry) -> None:
        ...
        info = get_protocol_identifiers_registry().get_protocol_info("0x0100")
    ...
    def test_get_protocol_info_rfcomm(self, protocol_identifiers_registry: ProtocolIdentifiersRegistry) -> None:
        ...
        info = get_protocol_identifiers_registry().get_protocol_info("0x0003")
        if info:  # Only if YAML loaded
            assert isinstance(info, ProtocolInfo)
    ...
    def test_get_protocol_info_avdtp(self, protocol_identifiers_registry: ProtocolIdentifiersRegistry) -> None:
        ...
        info = get_protocol_identifiers_registry().get_protocol_info("0x0019")
        if info:  # Only if YAML loaded
            assert isinstance(info, ProtocolInfo)
    ...
    def test_get_protocol_info_bnep(self, protocol_identifiers_registry: ProtocolIdentifiersRegistry) -> None:
        ...
        info = get_protocol_identifiers_registry().get_protocol_info("0x000F")
        if info:  # Only if YAML loaded
            assert isinstance(info, ProtocolInfo)
    ...
    def test_get_protocol_info_by_name(self, protocol_identifiers_registry: ProtocolIdentifiersRegistry) -> None:
        ...
        info = get_protocol_identifiers_registry().get_protocol_info("L2CAP")
        if info:  # Only if YAML loaded
            assert isinstance(info, ProtocolInfo)
            assert info.name == "L2CAP"
            assert info.uuid.short_form.upper() == "0100"
            ...
            info_lower = get_protocol_identifiers_registry().get_protocol_info("l2cap")
            assert info_lower == info

        # Test not found
        info_none = get_protocol_identifiers_registry().get_protocol_info("Nonexistent Protocol")
    ...
    def test_get_protocol_info_by_name_method(self, protocol_identifiers_registry: ProtocolIdentifiersRegistry) -> None:
        ...
        info = get_protocol_identifiers_registry().get_protocol_info_by_name("RFCOMM")
        if info:  # Only if YAML loaded
            assert isinstance(info, ProtocolInfo)
            assert info.name == "RFCOMM"
            assert info.uuid.short_form.upper() == "0003"
            ...
            # Test case insensitive
            info_lower = get_protocol_identifiers_registry().get_protocol_info_by_name("rfcomm")
            assert info_lower == info

        # Test not found
        info_none = get_protocol_identifiers_registry().get_protocol_info_by_name("Nonexistent Protocol")
        assert info_none is None
    ...
    def test_get_protocol_info_by_bluetooth_uuid(
        ...
        info = get_protocol_identifiers_registry().get_protocol_info(bt_uuid)
        if info:  # Only if YAML loaded
            assert isinstance(info, ProtocolInfo)
            assert info.name == "L2CAP"
    ...
    def test_get_protocol_info_by_int(self, protocol_identifiers_registry: ProtocolIdentifiersRegistry) -> None:
        ...
        info = get_protocol_identifiers_registry().get_protocol_info("0003")
        if info:  # Only if YAML loaded
            assert isinstance(info, ProtocolInfo)
            assert info.name == "RFCOMM"

        # Test with L2CAP as integer (0x0100 = 256)
        info = get_protocol_identifiers_registry().get_protocol_info("0100")
        if info:  # Only if YAML loaded
            assert isinstance(info, ProtocolInfo)
            assert info.name == "L2CAP"
    ...
    def test_get_protocol_info_not_found(self, protocol_identifiers_registry: ProtocolIdentifiersRegistry) -> None:
        ...
        info = get_protocol_identifiers_registry().get_protocol_info("nonexistent")
        assert info is None

        info = get_protocol_identifiers_registry().get_protocol_info("0xFFFF")  # Not a protocol UUID
        assert info is None
    ...
    def test_is_known_protocol(self, protocol_identifiers_registry: ProtocolIdentifiersRegistry) -> None:
        ...
        # Known protocol UUID
        has_protocols = bool(get_protocol_identifiers_registry().get_all_protocols())
        assert get_protocol_identifiers_registry().is_known_protocol("0x0100") or not has_protocols  # L2CAP

        # Non-protocol UUID
        assert not get_protocol_identifiers_registry().is_known_protocol("0xFFFF")
    ...
    def test_uuid_formats(self, protocol_identifiers_registry: ProtocolIdentifiersRegistry) -> None:
        ...
        for fmt in formats:
            info = get_protocol_identifiers_registry().get_protocol_info(fmt)
            if get_protocol_identifiers_registry().is_known_protocol("0100"):
    ...
    def test_well_known_protocols(self, protocol_identifiers_registry: ProtocolIdentifiersRegistry) -> None:
        ...
        for name, uuid in expected_protocols.items():
            info = get_protocol_identifiers_registry().get_protocol_info(name)
