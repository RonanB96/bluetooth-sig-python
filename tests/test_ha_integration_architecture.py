...
class TestHATranslationLayer:
    ...
    def test_gatt_to_ha_translation_temperature(self):
        ...
        assert (
            ha_data["device_class"] == "temperature"
    ...
    def test_gatt_to_ha_translation_illuminance(self):
        ...
        assert (
            ha_data["device_class"] == "illuminance"
        ), "Device class should be illuminance"
        assert (
            ha_data["state_class"] == "measurement"
        ), "State class should be measurement"
    ...
    def test_icon_mapping(self):
        ...
        assert "thermometer" in GATTSensorTranslator._get_icon_for_device_class(
            "temperature"
        )
        assert "battery" in GATTSensorTranslator._get_icon_for_device_class("battery")
        assert "water-percent" in GATTSensorTranslator._get_icon_for_device_class(
            "humidity"
        )
        assert "brightness" in GATTSensorTranslator._get_icon_for_device_class(
            "illuminance"
        )
    ...
    def test_batch_translation(self):
        ...
        characteristics = {"2A19": battery_char, "2A6E": temp_char}
        ...
        assert (
            abs(translated["2A6E"]["value"] - 19.24) < 0.01
        ), "Temperature should be approximately 19.24°C"
    ...
    def test_error_handling_in_translation(self):
        ...
        ha_data = GATTSensorTranslator.translate_to_ha_sensor(
            battery_char, invalid_data
        )
    ...
    def test_architecture_separation_validation(self):
        ...
        assert (
            "from homeassistant" not in translation_source
        ), "Translation layer should not import HA directly"
        assert (
            "import homeassistant" not in translation_source
        ), "Translation layer should not import HA directly"
