...
from bluetooth_sig.registry.uuids.mesh_profiles import MeshProfilesRegistry, get_mesh_profiles_registry
...
class TestMeshProfilesRegistry:
    ...
    def test_registry_initialization(self, mesh_profiles_registry: MeshProfilesRegistry) -> None:
        ...
        mesh_profiles = get_mesh_profiles_registry().get_all_mesh_profiles()
    ...
    def test_get_mesh_profile_info_by_name(self, mesh_profiles_registry: MeshProfilesRegistry) -> None:
        ...
        info = get_mesh_profiles_registry().get_mesh_profile_info_by_name("Generic OnOff Server")
        if info:  # Only if YAML loaded
            assert isinstance(info, MeshProfileInfo)
            assert info.name == "Generic OnOff Server"
        ...
        info_lower = get_mesh_profiles_registry().get_mesh_profile_info_by_name("generic onoff server")
        assert info_lower == info

        # Test not found
        info_none = get_mesh_profiles_registry().get_mesh_profile_info_by_name("Nonexistent Mesh Profile")
    ...
    def test_get_mesh_profile_info_by_bluetooth_uuid(self, mesh_profiles_registry: MeshProfilesRegistry) -> None:
        ...
        info = get_mesh_profiles_registry().get_mesh_profile_info(bt_uuid)
        if info:  # Only if YAML loaded
            assert isinstance(info, MeshProfileInfo)
            assert info.name == "Generic OnOff Server"
    ...
    def test_get_mesh_profile_info_not_found(self, mesh_profiles_registry: MeshProfilesRegistry) -> None:
        ...
        info = get_mesh_profiles_registry().get_mesh_profile_info("nonexistent")
        assert info is None

        info = get_mesh_profiles_registry().get_mesh_profile_info("0x0000")  # Not a mesh profile UUID
        assert info is None
    ...
    def test_is_mesh_profile_uuid(self, mesh_profiles_registry: MeshProfilesRegistry) -> None:
        ...
        has_profiles = bool(get_mesh_profiles_registry().get_all_mesh_profiles())
        assert get_mesh_profiles_registry().is_mesh_profile_uuid("0x1000") or not has_profiles

        # Non-mesh profile UUID
        assert not get_mesh_profiles_registry().is_mesh_profile_uuid("0x0000")

        # Invalid UUID
        assert not get_mesh_profiles_registry().is_mesh_profile_uuid("invalid")
    ...
    def test_uuid_formats(self, mesh_profiles_registry: MeshProfilesRegistry) -> None:
        ...
        for fmt in formats:
            info = get_mesh_profiles_registry().get_mesh_profile_info(fmt)
            if get_mesh_profiles_registry().is_mesh_profile_uuid("1000"):
