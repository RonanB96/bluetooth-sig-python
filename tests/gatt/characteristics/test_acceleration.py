...
class TestAccelerationCharacteristic(CommonCharacteristicTests):
    ...
    def test_acceleration_precision_and_boundaries(self, characteristic: BaseCharacteristic) -> None:
        ...
        result = characteristic.decode_value(bytearray([0x52, 0x26, 0x00, 0x00]))  # 9810 = 9.81 m/s²
        assert abs(result - 9.81) < 0.001

        # Test negative acceleration (-5.0 m/s²)
        result = characteristic.decode_value(bytearray([0x78, 0xEC, 0xFF, 0xFF]))  # -5000 = -5.0 m/s²
        assert abs(result + 5.0) < 0.001

    def test_acceleration_extreme_values(self, characteristic: BaseCharacteristic) -> None:
    ...
    def test_encode_value(self, characteristic: AccelerationCharacteristic) -> None:
        ...
        encoded = characteristic.encode_value(9.81)
        ...
        encoded = characteristic.encode_value(-5.0)
