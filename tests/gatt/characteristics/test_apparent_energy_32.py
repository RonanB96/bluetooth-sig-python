...
class TestApparentEnergy32Characteristic(CommonCharacteristicTests):
    ...
    def test_apparent_energy_32_precision_and_boundaries(self, characteristic: BaseCharacteristic) -> None:
        ...
        # Test maximum energy
        result = characteristic.decode_value(bytearray([0xFD, 0xFF, 0xFF, 0xFF]))  # 4294967293 = 4294967.293 kVAh
        assert abs(result - 4294967.293) < 0.001

    def test_apparent_energy_32_special_values(self, characteristic: BaseCharacteristic) -> None:
        """Test special values for apparent energy 32."""
        # Test "value is not valid" (0xFFFFFFFE)
        result = characteristic.decode_value(bytearray([0xFE, 0xFF, 0xFF, 0xFF]))
        assert result is None

        # Test "value is not known" (0xFFFFFFFF)
        result = characteristic.decode_value(bytearray([0xFF, 0xFF, 0xFF, 0xFF]))
        assert result is None

    def test_apparent_energy_32_extreme_values(self, characteristic: BaseCharacteristic) -> None:
    ...
    def test_encode_value(self, characteristic: ApparentEnergy32Characteristic) -> None:
        ...
        encoded = characteristic.encode_value(0.001)
