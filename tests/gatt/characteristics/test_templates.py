...
class TestEnumTemplateBasics:
    ...
    def test_decode_simple_enum_success(self) -> None:
        ...
        assert template._decode_value(bytearray([0x00])) == SimpleStatus.IDLE
        assert template._decode_value(bytearray([0x01])) == SimpleStatus.ACTIVE
        assert template._decode_value(bytearray([0x02])) == SimpleStatus.ERROR
        assert template._decode_value(bytearray([0x03])) == SimpleStatus.COMPLETED
    ...
    def test_decode_with_offset(self) -> None:
        ...
        result = template._decode_value(data, offset=2)
    ...
    def test_encode_enum_instance(self) -> None:
        ...
        assert template._encode_value(SimpleStatus.IDLE) == bytearray([0x00])
        assert template._encode_value(SimpleStatus.ACTIVE) == bytearray([0x01])
        assert template._encode_value(SimpleStatus.ERROR) == bytearray([0x02])

    def test_encode_int_value(self) -> None:
        """Test encoding from integer values."""
        template = EnumTemplate.uint8(SimpleStatus)

        assert template._encode_value(0) == bytearray([0x00])
        assert template._encode_value(1) == bytearray([0x01])
        assert template._encode_value(2) == bytearray([0x02])
    ...
    def test_round_trip(self) -> None:
        ...
        for status in SimpleStatus:
            encoded = template._encode_value(status)
            decoded = template._decode_value(encoded)
...
class TestEnumTemplateErrorHandling:
    ...
    def test_decode_insufficient_data(self) -> None:
        ...
        with pytest.raises(InsufficientDataError) as exc_info:
            template._decode_value(bytearray([]))
    ...
    def test_decode_invalid_enum_value(self) -> None:
        ...
        with pytest.raises(ValueRangeError) as exc_info:
            template._decode_value(bytearray([0xFF]))
    ...
    def test_decode_with_offset_insufficient_data(self) -> None:
        ...
        with pytest.raises(InsufficientDataError):
            template._decode_value(data, offset=5)
...
class TestEnumTemplateNonContiguous:
    ...
    def test_decode_non_contiguous_valid(self) -> None:
        ...
        assert template._decode_value(bytearray([0x00])) == NonContiguousEnum.FIRST
        assert template._decode_value(bytearray([0x02])) == NonContiguousEnum.SECOND
        assert template._decode_value(bytearray([0x05])) == NonContiguousEnum.THIRD
        assert template._decode_value(bytearray([0x0A])) == NonContiguousEnum.FOURTH
    ...
    def test_decode_non_contiguous_invalid_gap(self) -> None:
        ...
        with pytest.raises(ValueRangeError) as exc_info:
            template._decode_value(bytearray([0x01]))

        assert exc_info.value.value == 1

        # Value 3 is in a gap (between 2 and 5)
        with pytest.raises(ValueRangeError) as exc_info:
            template._decode_value(bytearray([0x03]))
    ...
    def test_encode_non_contiguous(self) -> None:
        ...
        assert template._encode_value(NonContiguousEnum.FIRST) == bytearray([0x00])
        assert template._encode_value(NonContiguousEnum.SECOND) == bytearray([0x02])
        assert template._encode_value(NonContiguousEnum.THIRD) == bytearray([0x05])
        assert template._encode_value(NonContiguousEnum.FOURTH) == bytearray([0x0A])
...
class TestEnumTemplateFactoryMethods:
    ...
    def test_uint16_factory(self) -> None:
        ...
        assert template._decode_value(bytearray([0x00, 0x01])) == LargeEnum.LOW
        assert template._encode_value(LargeEnum.HIGH) == bytearray([0x00, 0x03])
    ...
    def test_sint8_factory(self) -> None:
        ...
        assert template._decode_value(bytearray([0xF6])) == SignedEnum.NEGATIVE  # -10 as signed byte
        assert template._decode_value(bytearray([0x00])) == SignedEnum.ZERO
        assert template._decode_value(bytearray([0x0A])) == SignedEnum.POSITIVE
...
class TestEnumTemplateExplicitConstructor:
    ...
    def test_constructor_with_different_extractors(self) -> None:
        ...
        # Different encoding
        encoded_u8 = template_u8._encode_value(SimpleStatus.ACTIVE)
        encoded_u16 = template_u16._encode_value(SimpleStatus.ACTIVE)
...
class TestEnumTemplateEdgeCases:
    ...
    def test_single_value_enum(self) -> None:
        ...
        assert template._decode_value(bytearray([42])) == SingleValue.ONLY
        assert template._encode_value(SingleValue.ONLY) == bytearray([42])

        with pytest.raises(ValueRangeError):
            template._decode_value(bytearray([0]))
    ...
    def test_max_uint8_value(self) -> None:
        ...
        assert template._decode_value(bytearray([255])) == MaxValue.MAX
        assert template._encode_value(MaxValue.MAX) == bytearray([255])
    ...
    def test_zero_offset_explicit(self) -> None:
        ...
        result = template._decode_value(data, offset=0)
        assert result == SimpleStatus.ACTIVE
    ...
    def test_enum_with_zero_and_max(self) -> None:
        ...
        assert template._decode_value(bytearray([0x00])) == MinMax.MIN
        assert template._decode_value(bytearray([0xFF])) == MinMax.MAX

        # Middle value should fail
        with pytest.raises(ValueRangeError):
            template._decode_value(bytearray([0x80]))
...
class TestUtf8StringTemplate:
    ...
    def test_decode_empty_string(self) -> None:
        ...
        result = template._decode_value(bytearray())
        assert result == ""
    ...
    def test_decode_simple_string(self) -> None:
        ...
        result = template._decode_value(data)
        assert result == "Hello"
    ...
    def test_encode_string(self) -> None:
        ...
        template = Utf8StringTemplate()
        result = template._encode_value("Hello")
        assert result == bytearray(b"Hello")

    def test_encode_empty_string(self) -> None:
        """Test encoding an empty string."""
        template = Utf8StringTemplate()
        result = template._encode_value("")
    ...
    def test_encode_too_long(self) -> None:
        ...
        with pytest.raises(ValueError, match="String too long"):
            template._encode_value("Hello World")
...
class TestUtf16StringTemplate:
    ...
    def test_encode_too_long(self) -> None:
        ...
        with pytest.raises(ValueError, match="String too long"):
            template._encode_value("Hello")  # 5 chars = 10 bytes
    ...
    def test_round_trip_encoding(self) -> None:
        ...
        for s in test_strings:
            encoded = template._encode_value(s)
            decoded = template._decode_value(encoded)
            assert decoded == s
