...
class TestForceCharacteristic(CommonCharacteristicTests):
    ...
    def test_force_precision_and_boundaries(self, characteristic: BaseCharacteristic) -> None:
        ...
        result = characteristic.decode_value(bytearray([0x18, 0xFC, 0xFF, 0xFF]))  # -1000 = -1.00 N
        assert abs(result + 1.0) < 0.001

        # Test precision (1 N)
        result = characteristic.decode_value(bytearray([0xE8, 0x03, 0x00, 0x00]))  # 1000 = 1.00 N
        assert abs(result - 1.0) < 0.001

    def test_force_extreme_values(self, characteristic: BaseCharacteristic) -> None:
    ...
    def test_encode_value(self, characteristic: ForceCharacteristic) -> None:
        ...
        encoded = characteristic.encode_value(1.0)
