...
class TestDewPointCharacteristic(CommonCharacteristicTests):
    ...
    def test_dew_point_boundary_values(self, characteristic: DewPointCharacteristic) -> None:
        ...
        assert characteristic.parse_value(data_min).value == -128.0

        # Maximum value (127°C)
        data_max = bytearray([SINT8_MAX])
        assert characteristic.parse_value(data_max).value == 127.0
    ...
    def test_dew_point_typical_values(self, characteristic: DewPointCharacteristic) -> None:
        ...
        assert characteristic.parse_value(data_cold).value == -20.0

        # Cool (10°C)
        data_cool = bytearray([10])
        assert characteristic.parse_value(data_cool).value == 10.0

        # Comfortable (25°C)
        data_comfortable = bytearray([25])
        assert characteristic.parse_value(data_comfortable).value == 25.0
