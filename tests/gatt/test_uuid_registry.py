...
class TestBluetoothUUID:
    ...
    def test_to_bytes_method(self) -> None:
        """Test to_bytes() method for binary representation."""
        # Test with short form UUID (big-endian)
        uuid_short = BluetoothUUID("180F")  # Battery Service
        expected_bytes = bytes.fromhex("0000180f00001000800000805f9b34fb")
        assert uuid_short.to_bytes("big") == expected_bytes
        assert len(uuid_short.to_bytes("big")) == 16

        # Test with full form UUID
        uuid_full = BluetoothUUID("0000180F-0000-1000-8000-00805F9B34FB")
        assert uuid_full.to_bytes("big") == expected_bytes

        # Verify both forms produce same bytes
        assert uuid_short.to_bytes("big") == uuid_full.to_bytes("big")
        ...
        assert custom_uuid.to_bytes("big") == expected_custom

    def test_to_bytes_little_endian(self) -> None:
        ...
        assert uuid_short.to_bytes("little") == expected_bytes_le
        assert len(uuid_short.to_bytes("little")) == 16

        # Test that big-endian and little-endian are different
        assert uuid_short.to_bytes("big") != uuid_short.to_bytes("little")

        # Test that reversing little-endian gives big-endian
        assert bytes(reversed(uuid_short.to_bytes("little"))) == uuid_short.to_bytes("big")
