...
from bluetooth_sig.registry.uuids.members import MembersRegistry, get_members_registry
...
class TestMembersRegistry:
    ...
    def test_registry_initialization(self, members_registry: MembersRegistry) -> None:
        ...
        members = get_members_registry().get_all_members()
    ...
    def test_get_member_info_by_uuid(self, members_registry: MembersRegistry) -> None:
        ...
        name = get_members_registry().get_member_name("0xFEFF")
    ...
    def test_get_member_info_by_name(self, members_registry: MembersRegistry) -> None:
        ...
        info = get_members_registry().get_member_info_by_name("GN Netcom")
        ...
        info_lower = get_members_registry().get_member_info_by_name("gn netcom")
        assert info_lower == info

        # Test not found
        info_none = get_members_registry().get_member_info_by_name("Nonexistent Company")
    ...
    def test_get_member_info_by_bluetooth_uuid(self, members_registry: MembersRegistry) -> None:
        ...
        name = get_members_registry().get_member_name(bt_uuid)
        if name:  # Only if YAML loaded
            assert isinstance(name, str)
            assert name == "GN Netcom"
    ...
    def test_get_member_info_not_found(self, members_registry: MembersRegistry) -> None:
        ...
        name = get_members_registry().get_member_name("nonexistent")
        assert name is None

        name = get_members_registry().get_member_name("0x0000")  # Not a member UUID
        assert name is None

    def test_is_member_uuid(self, members_registry: MembersRegistry) -> None:
        """Test member UUID validation."""
        # Known member UUID
        assert get_members_registry().is_member_uuid("0xFEFF") or not get_members_registry().get_all_members()

        # Non-member UUID
        assert not get_members_registry().is_member_uuid("0x0000")

        # Invalid UUID
        assert not get_members_registry().is_member_uuid("invalid")
    ...
    def test_uuid_formats(self, members_registry: MembersRegistry) -> None:
        ...
        for fmt in formats:
            name = get_members_registry().get_member_name(fmt)
            if get_members_registry().is_member_uuid("FEFF"):
                assert name == "GN Netcom"
