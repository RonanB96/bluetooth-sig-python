...
class TestCoreFunctionality:
    ...
    def test_device_instantiation(self):
        ...
        ), f"Expected address {test_address}, got {device.address}"
    ...
    def test_device_methods_exist(self):
        ...
        assert hasattr(
            device, "read_characteristics"
        ), "Device should have read_characteristics method"
        assert hasattr(
            device, "read_parsed_characteristics"
        ), "Device should have read_parsed_characteristics method"
        assert hasattr(
            device, "get_device_info"
        ), "Device should have get_device_info method"

        # Test that methods are callable
        assert callable(
            device.read_characteristics
        ), "read_characteristics should be callable"
        assert callable(
            device.read_parsed_characteristics
        ), "read_parsed_characteristics should be callable"
    ...
    def test_device_string_representation(self):
        ...
        assert (
            test_address in device_str
        ), f"Device string representation should contain address {test_address}"
    ...
    async def test_core_methods_are_async(self):
        ...
        assert inspect.iscoroutinefunction(
            device.read_characteristics
        ), "read_characteristics should be async"
        assert inspect.iscoroutinefunction(
            device.read_parsed_characteristics
        ), "read_parsed_characteristics should be async"
    ...
    def test_multiple_device_instances(self):
        ...
        ), "Device addresses should be different"
    ...
    def test_device_interface_completeness(self):
        ...
        for method_name in required_methods:
            assert hasattr(
                device, method_name
            ), f"Device should have {method_name} method"
    ...
    def test_device_ready_for_real_testing(self):
        ...
        ), "Device should store the provided address"
        ...
        assert hasattr(
            device, "read_characteristics"
        ), "Device needs read_characteristics for real testing"
        assert hasattr(
            device, "read_parsed_characteristics"
        ), "Device needs read_parsed_characteristics for real testing"
